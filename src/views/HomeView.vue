...
<template>
  <div class="flex h-[100dvh] overflow-hidden bg-slate-50 text-slate-900 dark:bg-slate-900 dark:text-white">
    <AppSidebar />

    <div class="flex min-w-0 flex-1 flex-col lg:pl-[var(--app-sidebar-width)]">
      <header class="flex flex-col gap-4 px-4 py-6 lg:px-10">
      ...
      <div class="flex min-h-0 flex-1 flex-col">
        <main class="flex flex-1 min-h-0 flex-col gap-8 overflow-y-auto px-4 lg:flex-row lg:gap-8 lg:overflow-hidden lg:px-10">
          <section class="flex min-h-0 flex-1 flex-col overflow-hidden">
            <div class="flex-1 min-h-0 overflow-y-auto pb-6">
              <div class="mb-4 flex flex-wrap items-center gap-3 text-sm text-slate-500 dark:text-white/70">
              ...
              <div class="flex flex-col gap-12">
                <section>
                  ...
                  <div class="mt-5 grid auto-rows-auto grid-cols-[repeat(auto-fill,minmax(150px,1fr))] gap-3 sm:gap-4">
                    ...
                    >
                      ...
                      <p class="line-clamp-2 text-sm text-slate-500 dark:text-white/70">{{ item?.desc ?? '官方推荐' }}</p>
                    </button>
                  </div>
                </section>
                ...
                <section>
                  ...
                  <div class="mt-5 grid grid-cols-[repeat(auto-fill,minmax(200px,1fr))] justify-items-start gap-x-6 gap-y-5">
                    <button
                      ...
                      @click="handleOpenList(item)"
                    >
                      <img :src="item.image ?? 'https://placehold.co/300x300?text=No+Cover'" :alt="item.name ?? '歌单封面'" class="aspect-square w-full rounded-xl object-cover" />
                      <p class="mt-3 text-base font-semibold leading-tight">{{ item.name }}</p>
                      <p class="line-clamp-2 text-sm text-slate-500 dark:text-white/70">{{ item.desc || '精选歌单' }}</p>
                    </button>
                ...
                <section class="pb-4">
                  ...
                  <div class="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
                    ...
                    >
                      ...
                      >
                        <img :src="item.pic ?? 'https://placehold.co/112x112?text=Song'" :alt="item.name ?? '推荐歌曲'" class="h-14 w-14 flex-none rounded-md object-cover" />
                        <div class="min-w-0">
